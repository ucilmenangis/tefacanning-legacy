...
<div class="bg-white border border-gray-100 rounded-xl overflow-visible">
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 px-4 py-3 border-b border-gray-50">
        ...
        <div class="relative">
            ...
            <div id="filter-menu" class="hidden absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl z-[100] py-1.5">
                ...
                <button onclick="setStatusFilter('has_orders')" class="w-full text-left px-4 py-2 text-[12.5px] text-gray-600 hover:bg-gray-50 flex items-center gap-2">
                    Pernah Order
                </button>
                <button onclick="setStatusFilter('no_orders')" class="w-full text-left px-4 py-2 text-[12.5px] text-gray-600 hover:bg-gray-50 flex items-center gap-2">
                    Belum Order
                </button>
            </div>
        </div>
        ...
        <table class="w-full text-left text-[12.5px] border-collapse">
            ...
            <tbody>
                <?php foreach ($customers as $row): ?>
                    <tr class="customer-row transition-colors hover:bg-gray-50/50" data-has-orders="<?php echo $row['order_count'] > 0 ? 'yes' : 'no'; ?>" data-name="<?php echo strtolower($row['name']); ?>" data-org="<?php echo strtolower($row['organization'] ?? ''); ?>">
                        ...
                        <td class="px-3.5 py-3 border-b border-gray-50/50 align-middle text-gray-500">
                            <div class="flex items-center gap-1.5"><i class="ph ph-phone text-gray-300"></i> <?php echo htmlspecialchars($row['phone']); ?></div>
                        </td>
                        <td class="px-3.5 py-3 border-b border-gray-50/50 align-middle text-gray-500">
                            <div class="flex items-center gap-1.5"><i class="ph ph-envelope text-gray-300"></i> <?php echo htmlspecialchars($row['email']); ?></div>
                        </td>
                        <td class="px-3.5 py-3 border-b border-gray-50/50 align-middle">
    ...
    <div id="cards-view" class="hidden p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php foreach ($customers as $row): ?>
                <div class="customer-card bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all hover:-translate-y-1" data-has-orders="<?php echo $row['order_count'] > 0 ? 'yes' : 'no'; ?>" data-name="<?php echo strtolower($row['name']); ?>" data-org="<?php echo strtolower($row['organization'] ?? ''); ?>">
                    <div class="flex items-center justify-between mb-4">
                        <div class="w-10 h-10 rounded-full bg-primary/5 text-primary flex items-center justify-center font-bold text-sm">
                            <?php echo strtoupper(substr($row['name'], 0, 1)); ?>
                        </div>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold <?php echo $row['order_count'] > 0 ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-gray-50 text-gray-400 border border-gray-100'; ?>">
                            <?php echo $row['order_count']; ?> PESANAN
                        </span>
                    </div>

                    <div class="mb-4">
                        <div class="text-[16px] font-bold text-navy mb-0.5"><?php echo htmlspecialchars($row['name']); ?></div>
                        <div class="text-[12px] text-gray-400"><?php echo htmlspecialchars($row['organization'] ?: 'Personal'); ?></div>
                    </div>

                    <div class="space-y-2 mb-4">
                        <div class="flex items-center gap-2 text-[12px] text-gray-600">
                            <i class="ph ph-phone text-gray-300"></i> <?php echo htmlspecialchars($row['phone'] ?: '-'); ?>
                        </div>
                        <div class="flex items-center gap-2 text-[12px] text-gray-600">
                            <i class="ph ph-envelope text-gray-300"></i> <?php echo htmlspecialchars($row['email'] ?: '-'); ?>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 pt-4 border-t border-gray-50">
                        <a href="edit-customer.php?id=<?php echo $row['id']; ?>" class="flex-1 h-9 rounded-lg bg-gray-50 text-slate-600 text-[12px] font-bold flex items-center justify-center hover:bg-primary hover:text-white transition-all"><i class="ph ph-note-pencil mr-1.5"></i> Edit</a>
                        <button onclick="confirmDelete(<?php echo $row['id']; ?>)" class="w-9 h-9 rounded-lg bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors"><i class="ph ph-trash"></i></button>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <div id="empty-state" class="hidden py-20 text-center">
        ...
        <h3 class="text-[16px] font-bold text-navy mb-1">No customers found</h3>
    ...
    <div id="pagination-footer" class="px-4 py-3 border-t border-gray-50 flex items-center justify-between text-[12px] text-gray-400 gap-3 flex-wrap">
        <span id="results-count">Showing <?php echo count($customers); ?> results</span>
...
<script>
    ...
    function applyFilters() {
        ...
        let matchCount = 0;
        let visibleCount = 0;
        const items = (currentView === 'table') ? document.querySelectorAll('.customer-row') : document.querySelectorAll('.customer-card');
        
        document.querySelectorAll('.customer-row, .customer-card').forEach(el => el.classList.add('hidden'));

        items.forEach(el => {
            let filterMatch = true;
            if (currentFilter === 'has_orders') filterMatch = (el.dataset.hasOrders === 'yes');
            if (currentFilter === 'no_orders') filterMatch = (el.dataset.hasOrders === 'no');
